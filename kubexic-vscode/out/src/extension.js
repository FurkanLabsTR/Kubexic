...
function activate(context) {
    ...
    // Hover provider
    context.subscriptions.push(vscode.languages.registerHoverProvider("kubexic", {
    ...
    // Definition provider
    context.subscriptions.push(vscode.languages.registerDefinitionProvider("kubexic", {
    ...
    // Completion provider
    context.subscriptions.push(vscode.languages.registerCompletionItemProvider("kubexic", {
    ...
    // Check command
    context.subscriptions.push(vscode.commands.registerCommand("kubexic.check", () => {
        const editor = vscode.window.activeTextEditor;
        if (editor)
            checkProject(editor.document);
    }));
    // Check initial files
...
async function checkProject(changedDocument) {
...
function copyKxProject(sourceRoot, targetRoot) {
...
function parseErrors(output, checkRoot, originalRoot, changedFile, changedText) {
    ...
    for (const line of output.split("\n")) {
        ...
        if (fileMatch || localMatch) {
            ...
            try {
                ...
                startCol = realCol;
                while (startCol > 0 && /[a-zA-Z0-9_.]/.test(errorLine[startCol - 1]))
                    startCol--;
                // Walk forward to find end of token
                endCol = realCol;
                while (endCol < errorLine.length && /[a-zA-Z0-9_.]/.test(errorLine[endCol]))
                    endCol++;
                if (endCol === startCol)
                    endCol = startCol + 1;
            }
            catch { }
            const diag = new vscode.Diagnostic(new vscode.Range(realLine, startCol, realLine, endCol), message, sev);
            diag.source = "kubexic";
...
function scanProject(root) {
    ...
    const scan = (dir) => {
        ...
        for (const e of entries) {
            ...
            if (e.isDirectory() && !e.name.startsWith(".") && e.name !== "node_modules")
                scan(full);
            else if (e.isFile() && e.name.endsWith(".kx"))
                scanFile(full);
        }
    };
    const scanFile = (file) => {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if ((m = /\benum\s+([A-Z][a-zA-Z0-9_]*)/.exec(lines[i]))) {
                const bs = content.indexOf("{", m.index);
                const be = bs >= 0 ? content.indexOf("}", bs) : -1;
            ...
            if ((m = /\b(var|void|int|long|float|double|bool|byte|string)\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*\(/.exec(lines[i]))) {
                if (m[2] !== "main") {
                    const ps = content.indexOf("(", m.index);
                    const pe = ps >= 0 ? content.indexOf(")", ps) : -1;
    ...
    const extractFields = (c, si) => {
        const fields = [];
        const bs = c.indexOf("{", si);
        if (bs < 0)
            return fields;
        let d = 1, i = bs + 1, cl = "";
...
function getHoverInfo(word, document) {
    ...
    if (sym.kind === "component")
        code = `component ${sym.name} {\n${(sym.fields || []).map(f => `    var ${f} = ...;`).join("\n")}\n}`;
    else if (sym.kind === "system")
        code = `system ${sym.name}`;
    else if (sym.kind === "tag")
        code = `tag ${sym.name}${sym.parent ? " : " + sym.parent : ""}`;
    else if (sym.kind === "struct")
        code = `struct ${sym.name} {\n${(sym.fields || []).map(f => `    var ${f} = ...;`).join("\n")}\n}`;
    else if (sym.kind === "enum")
        code = `enum ${sym.name} { ${(sym.members || []).join(", ")} }`;
    else if (sym.kind === "function")
        code = `var ${sym.name}(${(sym.params || []).join(", ")}) { ... }`;
...
function getDefinition(word, document) {
    ...
    if (targetSym) {
        return new vscode.Location(vscode.Uri.file(targetSym.file), new vscode.Position(targetSym.line, 0));
    }
...
const KEYWORDS = ["component", "system", "tag", "struct", "enum", "const", "extern", "var", "void", "int", "long", "float", "double", "bool", "byte", "string", "if", "else", "while", "for", "foreach", "in", "break", "continue", "return", "spawn", "despawn", "attach", "detach", "self", "new", "with", "without", "tags", "is", "exact", "using", "true", "false", "panic", "run", "switch", "case", "default"];
function getCompletions(parts, document) {
    const root = vscode.workspace.getWorkspaceFolder(document.uri)?.uri.fsPath || path.dirname(document.uri.fsPath);
    const symbols = scanProject(root);
    const items = [];
    if (parts.length >= 2) {
        const obj = parts[0];
        if (obj === "std")
            return STD_FUNCTIONS.map(f => new vscode.CompletionItem(f, vscode.CompletionItemKind.Function));
        if (obj === "spatial")
            return SPATIAL_FUNCTIONS.map(f => new vscode.CompletionItem(f, vscode.CompletionItemKind.Function));
        const sym = symbols.get(obj);
        if (sym?.fields)
            return sym.fields.map(f => new vscode.CompletionItem(f, vscode.CompletionItemKind.Field));
        return items;
    }
